...
--SELECT * FROM item n LEFT JOIN path e ON n.id = e.child WHERE e.parent = 1;

-- get all nodes connected to 1 (parent and child)
--SELECT * FROM item WHERE id IN (SELECT parent FROM path WHERE child = 1 UNION SELECT child FROM path WHERE parent = 1);
